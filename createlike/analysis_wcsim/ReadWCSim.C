...
void ReadWCSim(char *filename=NULL, bool verbose=false){
    char* wcsimdirenv;
    ...
    TH1F *likelihood = new TH1F("likelihood", "t-tof", 750, -50, 250);
    ...
    for (int ev=0; ev<nevent; ev++)
    {
        ...
        for (int index = 0 ; index < wcsimrootsuperevent->GetNumberOfEvents(); index++)
        {
            ...
            for (i=0;i<ncherenkovdigihits;i++)
            {
                ...
                float ttof = T - TMath::Sqrt((vtxX-pmtX)*(vtxX-pmtX)+(vtxY-pmtY)*(vtxY-pmtY)+(vtxZ-pmtZ)*(vtxZ-pmtZ))/21.58333;
